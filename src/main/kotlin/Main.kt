...
fun main(args: Array<String>) {
    ...
    val inputStream: InputStream = if (args.size == 1) FileInputStream(args[0]) else System.`in`;
    val outputStream: PrintStream = System.out;

    val greeting: NothingReader = NothingReader(inputStream, outputStream, "Добро пожаловать в лаботаторую работу №2 \"Численное решение нелинейных уравнений и систем\"\n");
    val equationOrSystemReader: IntReader = IntReader(inputStream, outputStream, "Что будем решать? (нужно ввести одно целоое число) {\n1 -- нелинейное уравнение\n2 -- система нелинейных уравнений\n}: ");
    ...
    when (equationOrSystemReader.int) {
        1 -> {
            val segmentReader: SegmentReader = SegmentReader(inputStream, outputStream, "Введите границы интервала поиска корня (левая_граница правая_граница): ");
            val accuracyReader: DoubleReader = DoubleReader(inputStream, outputStream, "Введите точность: ");
            val methodReader: EquationsMethodReader = EquationsMethodReader(inputStream, outputStream, "Выберите метод решения (нужно ввести одно целое число) {\n1 -- метод хорд\n2 -- метод ньютона\n3 -- метод простой итерации\n}: ");

            methodReader.read();
            val method: EquationSolvingMethod = methodReader.method;

            val functionReader: FunctionReader =
                if (method == simpleIterationMethod) FunctionReader(inputStream, outputStream, "Введите функцию f(x), для которой будет решено уравнение f(x)=x:\n");
                else FunctionReader(inputStream, outputStream, "Введите функцию f(x), для которой будет решено уравнение f(x)=0:\n");
        ...
        2 -> {
            val methodReader: SystemsOfEquationsMethodReader = SystemsOfEquationsMethodReader(inputStream, outputStream, "Выберите метод решения (нужно ввести одно целое число) {\n1 -- метод простой итерации\n}: ");
            val function1Reader: FunctionReader = FunctionReader(inputStream, outputStream, "Нужно ввести функции f(x, y) и g(x, y) для которых будет решена система f(x, y)=x and g(x, y)=y.\nВведите функцию f(x, y):\n");
            val function2Reader: FunctionReader = FunctionReader(inputStream, outputStream, "Введите функцию g(x, y):\n");
            val segment1Reader: SegmentReader = SegmentReader(inputStream, outputStream, "Введите границы интервала поиска корня по x (левая_граница правая_граница): ");
            val segment2Reader: SegmentReader = SegmentReader(inputStream, outputStream, "Введите границы интервала поиска корня по y (левая_граница правая_граница): ");
            val accuracyReader: DoubleReader = DoubleReader(inputStream, outputStream, "Введите точность: ");
            ...
            println(method.solve(function1, function2, segment1, segment2, accuracy).root1);
            println(method.solve(function1, function2, segment1, segment2, accuracy).root2);
        }
    }
